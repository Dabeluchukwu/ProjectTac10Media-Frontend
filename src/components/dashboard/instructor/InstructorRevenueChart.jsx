...
const InstructorRevenueChart = ({ period = "month" }) => {
  ...
  // Responsive: fewer bars on mobile, show labels based on space
  const getDisplayData = () => {
    if (revenueData.length <= 7) return revenueData;
    // Show every 2nd item on mobile, but keep total count for desktop
    return revenueData;
  };
        ...
        {displayData.map((item, index) => (
          <div key={index} className="flex-1 flex flex-col items-center min-w-0">
            <div
              ...
              style={{
                height: `${(item.amount / maxRevenue) * 100}%`,
                minHeight: item.amount > 0 ? "6px" : "0px",
              }}
            ></div>
